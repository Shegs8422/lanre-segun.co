...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isOpen)) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "fixed inset-0 z-100 flex items-center justify-center bg-black/80 backdrop-blur-sm" }, _attrs))} data-v-508129e8><div class="bg-neutral-900 border border-neutral-800 p-8 rounded-2xl w-full max-w-sm shadow-2xl relative" data-v-508129e8><button class="absolute top-4 right-4 text-neutral-500 hover:text-white" data-v-508129e8><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-508129e8><line x1="18" y1="6" x2="6" y2="18" data-v-508129e8></line><line x1="6" y1="6" x2="18" y2="18" data-v-508129e8></line></svg></button><h2 class="text-xl font-bold text-white mb-6" data-v-508129e8>${ssrInterpolate(unref(modalTitle))}</h2>`);
        if (!unref(isRecoveryMode)) {
          _push(`<form class="flex flex-col gap-4" data-v-508129e8><input${ssrRenderAttr("value", unref(password))} type="password" placeholder="Enter password..." autocomplete="current-password" class="bg-black/50 border border-neutral-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 transition-colors" data-v-508129e8><button type="submit" class="bg-white text-black font-medium py-3 rounded-lg hover:bg-neutral-200 transition-colors"${ssrIncludeBooleanAttr(unref(loading)) ? " disabled" : ""} data-v-508129e8>${ssrInterpolate(unref(loading) ? "Checking..." : "Enter CMS")}</button><button type="button" class="text-sm text-neutral-400 hover:text-white transition-colors mt-2" data-v-508129e8> Forgot Password? </button></form>`);
        } else {
          _push(`<div data-v-508129e8>`);
          _push(ssrRenderComponent(_component_PasswordRecovery, {
            onSuccess: handleRecoverySuccess,
            onCancel: ($event) => isRecoveryMode.value = false
          }, null, _parent));
          _push(`<button type="button" class="w-full text-sm text-neutral-400 hover:text-white transition-colors mt-4" data-v-508129e8> Back to Login </button></div>`);
        }
        if (unref(error)) {
          _push(`<p class="text-red-500 text-sm mt-4 text-center" data-v-508129e8>${ssrInterpolate(unref(error))}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
...
const LoginModal = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["__scopeId", "data-v-508129e8"]]), { __name: "LoginModal" });

export { LoginModal as default };
//# sourceMappingURL=LoginModal-DwJFP9LP.mjs.map
