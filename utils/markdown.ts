 ...
 *
 * UPDATE: Now supports pre-rendered HTML (e.g. from Tiptap) by detecting HTML tags.
 */
export const parseMarkdown = (content: string | null | undefined): string => {
  ...
  // Heuristic: If content starts with an HTML tag, assume it's from Tiptap/HTML source
  if (/^<[a-z]+/.test(trimmedContent) || trimmedContent.startsWith("<")) {
  ...
  const processInline = (text: string | undefined): string => {
    ...
    processed = processed.replace(/`(.*?)`/g, (_match, codeContent) => {
      ...
      codeMatches.push(
        `<code class="inline-code bg-blue-500/10 text-blue-400 px-1.5 py-0.5 rounded text-[0.9em] font-mono font-medium">${codeContent}</code>`,
      );
    ...
    processed = processed
      .replace(
        /\*\*(.*?)\*\*/g,
        '<strong class="text-foreground font-black">$1</strong>',
      )
      .replace(/\*(.*?)\*/g, '<em class="italic opacity-90">$1</em>')
      .replace(
        /\[(.*?)\]\((.*?)\)/g,
        '<a href="$2" class="text-blue-500 hover:text-blue-400 hover:underline font-bold transition-colors" target="_blank">$1</a>',
      );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("> ")) {
      if (!blockquoteActive) {
        closeContainers();
        html +=
          '<blockquote class="border-l-4 border-blue-500 pl-8 my-10 italic bg-blue-500/5 py-6 pr-6 rounded-r-2xl shadow-xl"><div class="flex flex-col gap-4 text-xl text-foreground font-medium opacity-90">';
